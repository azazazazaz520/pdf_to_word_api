...
class ServiceConfig:
    ...
    embedded_image_max_pixels: int = _env_int(
        "PDF_SERVICE_EMBEDDED_IMAGE_MAX_PIXELS", 6_000_000
    )
    embedded_image_jpeg_quality: int = _env_int(
        "PDF_SERVICE_EMBEDDED_IMAGE_JPEG_QUALITY", 85
    )
    embedded_image_png_optimize: bool = _env_bool(
        "PDF_SERVICE_EMBEDDED_IMAGE_PNG_OPTIMIZE", False
    )
    render_validation: bool = _env_bool(
        "PDF_SERVICE_RENDER_VALIDATION", True
    )
    render_ssim: bool = _env_bool("PDF_SERVICE_RENDER_SSIM", True)
    render_ssim_dpi: float = _env_float("PDF_SERVICE_RENDER_SSIM_DPI", 110.0)
    render_text_compare: bool = _env_bool(
    ...
    fidelity_fallback_max_pixels: int = _env_int(
        "PDF_SERVICE_FIDELITY_FALLBACK_MAX_PIXELS", 2_000_000
    )
    fidelity_page_image_max_pixels: int = _env_int(
        "PDF_SERVICE_FIDELITY_PAGE_IMAGE_MAX_PIXELS", 8_000_000
    )
    ...
    page_delta_warn_ratio: float = _env_float(
        "PDF_SERVICE_PAGE_DELTA_WARN_RATIO", 0.05
    )
    page_delta_warn_absolute: int = _env_int(
        "PDF_SERVICE_PAGE_DELTA_WARN_ABSOLUTE", 3
    )
    render_timeout_seconds: int = _env_int(
        "PDF_SERVICE_RENDER_TIMEOUT_SECONDS", 180
    )
    include_toc: bool = _env_bool("PDF_SERVICE_INCLUDE_TOC", False)
    include_bookmarks: bool = _env_bool(
        "PDF_SERVICE_INCLUDE_BOOKMARKS", True
    )
    data_root: Path = Path(
    ...
    def __post_init__(self) -> None:
        ...
        if self.fidelity_fallback_dpi <= 0:
            raise RuntimeError("环境变量 PDF_SERVICE_FIDELITY_FALLBACK_DPI 必须大于 0")
        if self.fidelity_fallback_max_pixels < 1:
            raise RuntimeError(
                "环境变量 PDF_SERVICE_FIDELITY_FALLBACK_MAX_PIXELS 必须大于 0"
            )
        if self.fidelity_page_image_max_pixels < 1:
            raise RuntimeError(
                "环境变量 PDF_SERVICE_FIDELITY_PAGE_IMAGE_MAX_PIXELS 必须大于 0"
            )
        if not 1 <= self.page_image_jpeg_quality <= 100:
            raise RuntimeError(
                "环境变量 PDF_SERVICE_PAGE_IMAGE_JPEG_QUALITY 必须在 1 到 100 之间"
            )
        if self.embedded_image_max_pixels < 1:
            raise RuntimeError(
                "环境变量 PDF_SERVICE_EMBEDDED_IMAGE_MAX_PIXELS 必须大于 0"
            )
        if not 1 <= self.embedded_image_jpeg_quality <= 100:
            raise RuntimeError(
                "环境变量 PDF_SERVICE_EMBEDDED_IMAGE_JPEG_QUALITY 必须在 1 到 100 之间"
            )
        if self.render_timeout_seconds <= 0:
            raise RuntimeError(
                "环境变量 PDF_SERVICE_RENDER_TIMEOUT_SECONDS 必须大于 0"
            )
        if not 0 <= self.page_delta_warn_ratio <= 1:
            raise RuntimeError(
                "环境变量 PDF_SERVICE_PAGE_DELTA_WARN_RATIO 必须在 0 到 1 之间"
            )
        if self.page_delta_warn_absolute < 0:
            raise RuntimeError(
                "环境变量 PDF_SERVICE_PAGE_DELTA_WARN_ABSOLUTE 不能小于 0"
            )
...
class Job:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            ...
            "quality_gate": self.quality.get("quality_gate", {}),
            "route_summary": self.quality.get("route_summary", {}),
            "media_count": int(self.quality.get("media_count") or 0),
            "needs_review_pages": self.quality.get("needs_review_pages", []),
            "page_results": self.quality.get("page_results", []),
    ...
    def as_record(self) -> dict[str, Any]:
        ...
        return {
            ...
            "table_count": self.table_count,
            "quality_json": (
                json.dumps(self.quality, ensure_ascii=False)
                if self.quality
                else None
            ),
            "worker_pid": self.worker_pid,
...
class JobManager:
    ...
    def _job_from_record(record: dict[str, Any]) -> Job | None:
        try:
            return Job(
                ...
                quality=(
                    json.loads(str(record["quality_json"]))
                    if record.get("quality_json")
                    else {}
                ),
    ...
    def _worker_payload(self, job: Job) -> dict[str, Any]:
        return {
            ...
            "render_ssim": CONFIG.render_ssim,
            ...
            "font_metrics_cache": str(
                CONFIG.data_root / "font_metrics.json"
            ),
            "fidelity_auto_fallback": CONFIG.fidelity_auto_fallback,
            "fidelity_fallback_dpi": CONFIG.fidelity_fallback_dpi,
            "fidelity_fallback_max_pixels": CONFIG.fidelity_fallback_max_pixels,
            "fidelity_page_image_max_pixels": (
                CONFIG.fidelity_page_image_max_pixels
            ),
